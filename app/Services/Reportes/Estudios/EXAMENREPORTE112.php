...
class EXAMENREPORTE112 extends Reporte
{
...
include('variables.php');
        ...
        //presentacion
        $pdf->SetFont('Arial','',12);$pdf->SetXY(132,28);$pdf->Cell(0,4,$fechal,0,0,'L');$pdf->Ln(10);
        ...
        $pdf->SetFont('Arial','',11);$pdf->Cell(0,4,"DNI: ".$doc,0,0,'L');$pdf->Ln(16);
        //cuerpo
        $pdf->SetFont('Arial','B',11);$pdf->Cell(0,4,"CONSENTIMIENTO INFORMADO PARA EL TESTEO DE ALCOHOL Y DROGAS",0,0,'C');$pdf->Ln(10);
        $pdf->SetFont('Arial','',11);
        $pdf->SetLeftMargin(30);
        $pdf->MultiCell(150,5,"En mi condicion de empleado de Halliburton Argentina, doy mi consentimiento a la realizacion de la prueba de drogas o sustancias prohibidas: ".$nombreExamen,0,'J',0,5);$pdf->Ln();
        ...
        $y=$pdf->GetY();$pdf->SetY($y+16);$y=$pdf->GetY();
        $pdf->Line(130,$y,185,$y);
        $pdf->SetXY(130,$y+2);$pdf->Cell(55,3,'Firma',0,0,'C');$pdf->Ln(10);
        $pdf->Cell(0,10,"Nombre y apellido del empleado:",0,0,'L');$pdf->Ln();
        $y=$pdf->GetY();$pdf->SetY($y+16);$y=$pdf->GetY();
        $pdf->Line(130,$y,185,$y);
        $pdf->SetXY(130,$y+2);$pdf->Cell(55,3,'Firma',0,0,'C');
        //pag 2
        $pdf->AddPage();
        $pdf->Image(public_path("/archivos/reportes/E112_1.jpg"),20,15,180); 
        $pdf->Image(public_path("/archivos/reportes/E112_2.jpg"),20,86,180); 
        $pdf->Image(public_path("/archivos/reportes/E112_3.jpg"),20,206,180); 
        if (!$vistaPrevia) Tools::generarQR('A', $prestacion->Id, $datos['idExamen'], $prestacion->paciente->Id, "qr", $pdf);
        else $pdf->Image(Tools::generarQRPrueba('A', "qr"), 190, 15, 15, 15);
    }
