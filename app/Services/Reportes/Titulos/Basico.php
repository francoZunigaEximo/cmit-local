...
class Basico extends Reporte
{
    public function render(FPDF $pdf, $datos = ['detalles','id']): void
    {
        ...
        $pdf->SetX(10);
        $pdf->Cell(100,3,utf8_decode(ReporteConfig::$TITULO),0,0,'L');
        $pdf->Ln();
        $pdf->SetFont('Arial','',7);
        $pdf->SetX(10);
        $pdf->Cell(0,3, utf8_decode(ReporteConfig::$DIRECCION),0,0,'L');
        $pdf->Ln();
        $pdf->Line(10,26,200,26);
        $pdf->SetFont('Arial','B',14);
        $pdf->SetXY(10,9);
        $pdf->Cell(200,15, utf8_decode($datos['detalles']),0,0,'C');
        $pdf->SetFont('Arial','',9);
        $pdf->SetXY(10,28);
