...
class ConstEstCompDetallado extends Reporte
{
    ...
    public function render(FPDF $pdf, $datos = ['id']): void
    {
        ...
        //texto informe
	    $texto = "Por medio de la presente dejo constancia que en el dia de la fecha el Sr/a, ".utf8_decode($prestacion->paciente->Apellido." ".$prestacion->paciente->Nombre).", ".$prestacion->paciente->TipoDocumento.': '.$prestacion->paciente->Documento." ha completado su examen ".utf8_decode($tipoPrestacion['tipoExamen'])." para la empresa ".utf8_decode($prestacion->empresa->ParaEmpresa)." que consta de los siguientes estudios:";
        ...
        foreach ($itemsprestaciones as $item) { 
            $pdf->SetX(15);$pdf->Cell(0,3,'- '.utf8_decode(substr($item->examenes->Nombre,0,40)),0,0,'L');
            $pdf->SetX(87);$pdf->Cell(0,3,utf8_decode(substr($item->ObsExamen,0,75)),0,0,'L');$pdf->Ln(4);
        }
        $pdf->Ln(4);
    }
