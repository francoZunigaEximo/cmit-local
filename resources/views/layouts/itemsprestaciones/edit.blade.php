...
@section('content')
    ...
    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
        <h4 class="mb-sm-0">Exámen prestación <span
                class="custom-badge original">N°{{ $data['itemprestacion']->IdPrestacion }}</span> | Paciente <span
                class="custom-badge original">{{ $data['paciente']->paciente->Nombre ?? '' }}
                {{ $data['paciente']->Apellido ?? '' }}</span> {!! $data['itemprestacion']->Anulado === 1 ? '<span class="custom-badge rojo">Bloqueado</span>' : '' !!}</h4>
                ...
                <div class="row">
                    <div class="col-6">
                        <div class="input-group input-group-sm mb-2">
                            ...
                            <input type="text" class="form-control" name="provEfector" id="provEfector"
                                value="{{ $data['itemprestacion']->examenes->proveedor1->Nombre ?? '' }}" @readonly(true)>
                            <input type="hidden" id="IdEfector"
                                value="{{ $data['itemprestacion']->examenes->proveedor1->Id ?? '' }}">
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="input-group input-group-sm mb-2">
                            <span class="input-group-text">Especialidad Informador</span>
                            <input type="text" class="form-control" name="provInformador" id="provInformador"
                                value="{{ $data['itemprestacion']->examenes->proveedor2->Nombre ?? '' }}"
                                @readonly(true)>
                            <input type="hidden" id="IdInformador"
                                value="{{ $data['itemprestacion']->examenes->proveedor2->Id ?? '' }}">
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-2">
                        <div class="input-group input-group-sm mb-2">
                            ...
                            <input type="date" class="form-control" id="FechaAsignado" name="FechaAsignado"
                                value="{{ $data['itemprestacion']->FechaAsignado ?? '' }}" @readonly(true)>
                        </div>
                    </div>
                    ...
                    <div class="col-md-2">
                        <div class="input-group input-group-sm mb-2">
                            ...
                            <input type="text" class="form-control"
                                style="color: {{ (in_array($data['itemprestacion']->CAdj, [0, 1, 2]) ? 'red' : (in_array($data['itemprestacion']->CAdj, [3, 4, 5]) ? 'green' : '')) ?? '' }}"
                                id="Estado" name="Estado"
                                value="{{ (in_array($data['itemprestacion']->CAdj, [0, 1, 2]) ? 'Abierto' : (in_array($data['itemprestacion']->CAdj, [3, 4, 5]) ? 'Cerrado' : '')) ?? '' }}"
                                @readonly(true)>
                        </div>
                    </div>
                    ...
                    <div class="col-md-2">
                        <div class="input-group input-group-sm mb-2">
                            ...
                            <input type="date" class="form-control" id="FechaPagado" name="FechaPagado"
                                value="{{ $data['itemprestacion']->FechaPagado ?? '' }}" @readonly(true)>
                        </div>
                    </div>
                    @if ($data['prestacion'] === 0)
                        <div class="col-md-2">
                            {!! $data['itemprestacion']->Anulado === 1
                                ? ''
                                : '<button type="button" id="asignar" class="btn botonGeneral btn-sm asignar">Asignar</button>' !!}
                            <button type="button" id="liberar"
                                class="btn botonGeneral btn-sm liberar">Liberar</button>
                            {!! $data['itemprestacion']->Anulado === 1
                                ? ''
                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="input-group input-group-sm mb-2">
                            ...
                            <input type="text"
                                style="{{ $data['itemprestacion']->examenes->Adjunto === 1 && !empty($data['adjuntoEfector']) && $data['adjuntoEfector'] === 0 ? 'color: red' : ($data['itemprestacion']->examenes->Adjunto === 1 && !empty($data['adjuntoEfector']) && $data['adjuntoEfector'] === 1 ? 'color: green' : '') }}"
                                class="form-control" id="Estado" name="Estado"
                                value="{{ $data['itemprestacion']->examenes->Adjunto === 0 ? 'No lleva Adjuntos' : ($data['itemprestacion']->examenes->Adjunto === 1 && !empty($data['adjuntoEfector']) && $data['adjuntoEfector'] === 0 ? 'Pendiente' : ($data['itemprestacion']->examenes->Adjunto === 1 && !empty($data['adjuntoEfector']) && $data['adjuntoEfector'] === 1 ? 'Adjuntado' : '-')) }}"
                                @readonly(true)>
                            @if ($data['prestacion'] === 0)
                                <button type="button" class="btn botonGeneral adjuntarEfector" data-bs-toggle="modal"
                                    data-bs-target="#modalEfector">Adjuntar archivo</button>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                        ...
                        <div class="input-group input-group-sm mb-2">
                            @if ($data['itemprestacion']->CInfo !== 0)
                                ...
                                <select name="informadores" id="informadores" class="form-control">
                                    <option value="{{ $data['informadores']['id'] ?? '' }}" selected>
                                        {{ $data['informadores']['NombreCompleto'] ?? '' }}</option>
                                </select>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="col-md-2">
                        <div class="input-group input-group-sm mb-2">
                            ...
                            @if ($data['itemprestacion']->CInfo !== 0)
                                <span class="input-group-text">Estado</span>
                                <input type="text" class="form-control"
                                    style="color: {{ ($data['itemprestacion']->CInfo === 0 || $data['itemprestacion']->CInfo === 1 || $data['itemprestacion']->CInfo === 2 ? 'red' : ($data['itemprestacion']->CInfo === 3 ? 'green' : '')) ?? '' }}"
                                    id="EstadoI" name="EstadoI"
                                    value="{{ (in_array($data['itemprestacion']->CInfo, [0, 1]) ? 'Pediente' : ($data['itemprestacion']->CInfo === 2 ? 'Borrador' : ($data['itemprestacion']->CInfo === 3 ? 'Cerrado' : ''))) ?? '' }}"
                                    @readonly(true)>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="col-md-2">
                        @if ($data['itemprestacion']->CInfo !== 0)
                            <div class="input-group input-group-sm mb-2">
                                <span class="input-group-text">Fecha Pago</span>
                                <input type="date" id="FechaPagado2" name="FechaPagado2" class="form-control"
                                    value="{{ $data['itemprestacion']->FechaPagado2 }}" @readonly(true)>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="col-md-2">
                        @if ($data['prestacion'] === 0)
                            @if ($data['itemprestacion']->CInfo !== 0)
                                ...
                                <button type="button" id="liberarI"
                                    class="btn botonGeneral btn-sm liberarI">Liberar</button>
                                {!! $data['itemprestacion']->Anulado === 1
                                    ? ''
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <div class="input-group input-group-sm mb-2">
                                ...
                                <span class="input-group-text">Adjunto</span>
                                <input type="text"
                                    style="{{ $data['itemprestacion']->profesionales2->InfAdj === 1 && $data['adjuntoInformador'] === 0 ? 'color: red' : ($data['itemprestacion']->profesionales2->InfAdj === 1 && $data['adjuntoInformador'] === 1 ? 'color: green' : '') }}"
                                    class="form-control" id="EstadoInf" name="EstadoInf"
                                    value="{{ $data['itemprestacion']->profesionales2->InfAdj === 0 ? 'No lleva Adjuntos' : ($data['itemprestacion']->profesionales2->InfAdj === 1 && $data['adjuntoInformador'] === 0 ? 'Pendiente' : ($data['itemprestacion']->profesionales2->InfAdj === 1 && $data['adjuntoInformador'] === 1 ? 'Adjuntado' : '-')) }}"
                                    @readonly(true)>
                                @if ($data['prestacion'] === 0)
                                    <button type="button" class="btn botonGeneral adjuntarInformador"
                                        data-bs-toggle="modal" data-bs-target="#modalInformador">Adjuntar archivo</button>
                                @endif
                            </div>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="input-group input-group-sm mb-2">
                            ...
                            <input type="date" class="form-control" id="FechaFacturaVta" name="FechaFacturaVta"
                                value="" @readonly(true)>
                        </div>
                        <div class="input-group input-group-sm mb-2">
                            <span class="input-group-text">Nro Factura</span>
                            <input type="text" class="form-control" id="NroFacturaVta" name=" NroFacturaVta"
                                value="" @readonly(true)>
                        </div>
                    </div>
                    ...
                    <div class="col-md-4">
                        <div class="input-group input-group-sm mb-2">
                            ...
                            <input type="date" class="form-control" id="FechaNC" name="FechaNC"
                                value="{{ $data['itemprestacion']->notaCreditoIt->notaCredito->Fecha ?? '' }}"
                                @readonly(true)>
                        </div>
                        <div class="input-group input-group-sm mb-2">
                            <span class="input-group-text">Número NC</span>
                            <input type="text" class="form-control" id="NumeroNC" name="NumeroNC"
                                value="{{ $data['itemprestacion']->notaCreditoIt->notaCredito->Tipo ?? '' }}{{ $data['itemprestacion']->notaCreditoIt->notaCredito->Sucursal ?? '' }}{{ $data['itemprestacion']->notaCreditoIt->notaCredito->Nro ?? '' }}"
                                @readonly(true)>
                        </div>
                    </div>
    ...
    <div id="modalEfector" class="modal fade" tabindex="-1" aria-labelledby="myModalLabel1" aria-hidden="true"
        style="display: none;">
                ...
                <div class="modal-body">
                    <form id="form-efector">
                        ...
                        @if ($data['itemprestacion']->examenes->proveedor1->Multi == 1)
                            <div class="alert alert-info alert-border-left alert-dismissible fade show mb-2"
                                role="alert">
                                ...
                                @foreach ($data['multiEfector'] as $examen)
                                    <label class="list-group-item">
                                        <input class="form-check-input me-1" type="checkbox"
                                            id="Id_multiAdj_{{ $examen->Id }}" value="{{ $examen->Id }}"
                                            {{ $examen->archivos_count > 0 ? 'disabled' : 'checked' }}>
                                        {!! $examen->archivos_count > 0
                                            ? $examen->NombreExamen . ' <i title="Con archivo adjunto" class="ri-attachment-line verde"></i>'
                                            : $examen->NombreExamen !!}
                                    </label>
                                @endforeach
                            </div>
                        ...
                        <div class="mt-3">
                            ...
                            <input type="hidden" id="multi"
                                value="{{ $data['itemprestacion']->examenes->proveedor1->Multi == 1 ? 'success' : 'fail' }}">
                        </div>
                    </form>
                </div>
    ...
    <div id="replaceAdjunto" class="modal fade" tabindex="-1" aria-labelledby="myModalLabel3" aria-hidden="true"
        style="display: none">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
    ...
    <div id="modalInformador" class="modal fade" tabindex="-1" aria-labelledby="myModalLabel2" aria-hidden="true"
        style="display: none">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <form id="form-informador">
                        @if ($data['itemprestacion']->examenes->proveedor2->MultiE == 1 && $data['itemprestacion']->profesionales2->InfAdj == 1)
                            ...
                            <div class="list-group">
                                @foreach ($data['multiInformador'] as $informe)
                                    <label class="list-group-item">
                                        <input class="form-check-input me-1" type="checkbox"
                                            id="Id_multiAdjInf_{{ $informe->Id }}" value="{{ $informe->Id }}"
                                            {{ $informe->archivos_count > 0 ? 'disabled' : 'checked' }}>
                                        {!! $informe->archivos_count > 0
                                            ? ($informe->NombreExamen ?? '') .
                                                ' (' .
                                                ($informe->NombreProveedor ?? '') .
                                                ') <i title="Con archivo adjunto" class="ri-attachment-line verde"></i>'
                                            : ($informe->examenes->Nombre ?? '') . ' (' . ($informe->NombreProveedor ?? '') . ')' !!}
                        ...
                        <div class="mt-3">
                            ...
                            <input type="hidden" id="multiE"
                                value="{{ $data['itemprestacion']->examenes->proveedor2->MultiE == 1 && $data['itemprestacion']->profesionales2->InfAdj == 1 ? 'success' : 'fail' }}">
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
